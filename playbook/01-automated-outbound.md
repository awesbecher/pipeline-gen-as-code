...
cascades three to five providers until emails verify at 95 percent plus,
and AI research writes one specific opening line per contact from a real
signal. A tight 300-account list beats a loose 3,000 every time, because
reply rate compounds into deliverability. One operator runs 400 to 500
sends a day across the whole system; the rep only touches replies.
...
week six feels.
Weeks 9 to 12: scale toward full operator throughput. The rep works
replies only. Report on replies and meetings, never on sends.

## Tripwires

Replies sitting in the unenriched band (1 to 3 percent): stop and fix the
list or the angle before touching volume. Verification below the 95
percent bar: the waterfall is broken; no sends until it is not.
